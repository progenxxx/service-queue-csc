...
export const GET = requireRole(['super_admin', 'customer_admin', 'agent', 'agent_manager'])(
  async (req: NextRequest) => {
    try {
      ...
      // Build base where conditions (without status filter for totals)
      const baseConditions: any[] = [];

      // Customer filter
      if (params.customerId) {
        baseConditions.push(eq(serviceRequests.companyId, params.customerId));
      }
      ...
      if (dateFilter) {
        baseConditions.push(gte(serviceRequests.createdAt, dateFilter.start));
        baseConditions.push(lte(serviceRequests.createdAt, dateFilter.end));
      }

      // Get current totals (all statuses)
      ...
      // Use the same date filter for time series data
      let timeSeriesDateFilter: { start: Date; end: Date } = dateFilter || {
        start: new Date(currentDate.getTime() - 365 * 24 * 60 * 60 * 1000), // Default to last year
        end: currentDate
      };
      ...
      if (params.timeRange === 'daily') {
        ...
        for (let i = 0; i < maxDays; i++) {
          ...
          timeSeriesData.push({
            ...
            totalPastDue: 0, // Simplified for daily view
          });
        }
        ...
        for (let i = 0; i < maxWeeks; i++) {
          ...
          timeSeriesData.push({
            ...
            totalPastDue: 0, // Simplified for weekly view
          });
        }
      } else {
        ...
        for (let i = 0; i < maxMonths; i++) {
          ...
          timeSeriesData.push({
            ...
            totalPastDue: 0, // Simplified for now
          });
        }
      }
