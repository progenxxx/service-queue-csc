...
const getNavigation = (currentPath: string = '/agent/summary'): NavigationItem[] => [
  { 
    name: 'All Request', 
    href: '/agent', 
    icon: Home, 
...
const fetchMyRequests = async (): Promise<ServiceRequest[]> => {
  const response = await fetch('/api/agent/my-requests');
  if (!response.ok) {
  ...
  return (
    <DashboardLayout navigation={navigation} title="">
      <div className="space-y-6">
        ...
        <Card className="shadow-sm border-0 hidden lg:block">
